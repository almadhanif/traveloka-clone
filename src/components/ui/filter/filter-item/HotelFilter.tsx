  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentList((prevList) =>
        prevList === listTrustBy1 ? listTrustBy2 : listTrustBy1
      );
    ...
    <div className='flex justify-center items-center gap-x-7 p-2'>
      ...
      >
        {currentList.map((item) => (
          <div
            key={item.name}
            className='hover:bg-white text-white p-2 rounded-lg transition-transform transform hover:scale-105'
          >
            <Image className='text-white' src={item.img} alt={item.name} />
          </div>
        ))}
      </div>
    </div>
    ...
    <div className='w-full flex justify-center items-center'>
      <div className='flex flex-col w-[30%]'>
        ...
        <div className='relative w-full'>
          <CiLocationOn
            color='blue'
            className='absolute left-2 top-4 w-5 h-5'
          />
          <input
            id='Location'
            name='Location'
      ...
      <div className='flex flex-col w-[30%]'>
        ...
        <input
          ...
          className='h-14 border-y-[3px] border-gray-400 p-2'
        />
      </div>
      <div className='flex flex-col w-[30%]'>
        ...
        <div className='relative w-full'>
          ...
          <input
            ...
            className='h-14 border-l-2 border-[3px] border-gray-400 p-2 pl-9 w-full'
      ...
      <div className='w-[8%] h-full flex flex-col items-center justify-center'>
        ...
        <button
          ...
          className='h-14 w-full flex items-center justify-center bg-orange-500 hover:bg-orange-600 text-white rounded-r-xl focus:ring-2 focus:ring-orange-400 focus:outline-none'
        >
...
const HotelFilter = () => {
  ...
  const handleSearch = () => {
    const filtered = hotels.filter((hotel) => {
      const matchesLocation = hotel.name
        .toLowerCase()
      ...
      <div className='flex items-center justify-start gap-x-2'>
        {listCategories.map((item: ListCategoriesType) => (
          <div
            key={item.label}
            className={`${
              activeCategories === item.label ? 'bg-blue-500' : 'bg-black/30'
            } px-3 py-1 rounded-full text-white font-semibold flex items-center justify-center gap-x-2 cursor-pointer`}
