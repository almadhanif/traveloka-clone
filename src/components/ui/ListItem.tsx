  ...
  if (item.type === 'link') {
    return (
      <li>
        <a
          href={item.link}
          className={`${textColor} flex justify-center items-center`}
        >
          {item.icon && (
            <Image src={item.icon} alt={item.label} className='mr-2 w-5 h-5' />
          )}
          {item.label}
        </a>
      </li>
  ...
  if (item.type === 'dropdown' && item.multiSubItems) {
    return (
      <li className='relative'>
        <button
          onClick={onToggle}
          className={`${textColor} flex justify-center items-center gap-x-2`}
        >
                ...
                {multiSubItem.subItem.map((subItem, subIndex) => (
                  <li key={subIndex} className='hover:bg-gray-100'>
                    ...
                    >
                      <div className='flex items-center justify-start gap-x-2'>
                        ...
                        typeof subItem.icon === 'string' ? (
                          ...
                          <div
                            className={`${
                              multiSubItem.label !== 'Currency'
                                ? 'w-5 h-5 border-2 border-blue-500 rounded-full'
                                : 'w-7 h-7 font-semibold'
                      ...
                      {subItem.icon === activeLang && (
                        <CheckIcon className='w-4 h-4 text-blue-500' />
                      )}
                      {subItem.icon === activeCurrency && (
                        <CheckIcon className='w-4 h-4 text-blue-500' />
                      )}
                    </a>
                  </li>
