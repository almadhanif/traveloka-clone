...
const Navbar = () => {
  ...
  const listItems1: MenuItem[] = [
    ...
    { type: "link", icon: "", label: "Year End Sale", link: "/#" },
    {
      ...
      subItems: [
        {
          icon: <QuestionMarkCircleIcon className="w-5 h-5" color="white" stroke="black" />,
          label: "Help Center",
          link: "/#",
        },
        {
          icon: <ChatBubbleLeftEllipsisIcon className="w-5 h-5" color="white" stroke="black" />,
          label: "Contact Us",
          link: "/#",
        },
      ],
  ...
  return (
    <div className={`sticky top-0 w-full pt-2 z-10 ${isScrolled || theme === "dark" ? "bg-white" : "bg-gradient-to-b from-black/40 from-10% via-transparent via-60% to-transparent to-40%"}`}>
      <PaddingContainer>
        {/* nav list 1 */}
        <div className="w-full flex justify-between items-center gap-x-10 pb-2">
          ...
          <div className="w-[190px] z-20">
            <Link href={"/"}>
              <Image src={isScrolled || theme === "dark" ? TravelokaImgBlue : TravelokaImgWhite} alt="traveloka-img" width={500} height={500} className="w-full h-full object-cover" />
            </Link>
          </div>
          ...
          <div className="flex space-x-3 text-white w-auto">
            <Button text="Log in" variant={isScrolled || theme === "dark" ? "outlineBlack" : "outlineWhite"} icon={<UserIcon className={`w-4 h-4 transition-transform`} />} onClick={() => setModalOpen(true)} />
        ...
        <div className="py-2">
          ...
          <ul className="w-full flex justify-start items-center gap-x-2 font-semibold">
            {listItems2.map((item, index) => (
              <ListItem key={index} item={item} color={isScrolled || theme === "dark" ? "black" : "white"} isOpen={openDropdownIndex === index} onToggle={() => handleToggleDropdown(index)} />
            ))}
          </ul>
        </div>
      </PaddingContainer>
