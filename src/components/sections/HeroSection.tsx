...
const HeroSection = () => {
  ...
  const activeItem = activeSubFilter ? listFilters.find((filter) => filter.tabs === activeFilter)?.subItems?.find((sub) => sub.tabs === activeSubFilter)?.component : listFilters.find((filter) => filter.tabs === activeFilter)?.component;
  ...
  return (
    <div
      className="w-full h-[510px] -mt-28 flex justify-center items-center"
      style={{
            ...
            if (item.type === "tab") {
              return (
                <li
                  ...
                  className={`${
                    activeFilter === item.tabs ? "text-black bg-white rounded-full" : "text-gray-300 hover:border hover:border-white hover:rounded-full hover:text-gray-50"
                  } flex items-center justify-center cursor-pointer font-bold p-4 gap-x-2`}
                >
                  {item.icon}
                  {item.label}
                </li>
            ...
            if (item.type === "dropdown" && item.subItems) {
              return (
                <li key={item.label} className="relative">
                  ...
                  {/* Dropdown menu */}
                  {activeDropdown && (
                    <ul ref={dropdownRef} className="absolute flex bg-white shadow-md rounded-lg mt-2 w-max">
                      {item.subItems.map((subItem) => (
                        <li key={subItem.label} onClick={() => handleSubFilterClick(subItem)} className={`${activeSubFilter === subItem.tabs ? "bg-gray-200" : "hover:bg-gray-100"} cursor-pointer p-2`}>
                          {subItem.label}
                        </li>
                      ))}
                    </ul>
